...
namespace Actions
{
    public abstract class EventAction : basicAction
    {
        ...
        public void setActorMove(GameObject _player, Vector2 _move)
        {
            _player.GetComponent<ActorInterface>().Move(_move);
        }

        public turn getActorTurn(GameObject _player)
        {
            return _player.GetComponent<ActorInterface>().getTurn();
        }
