    ...
    public class DoEvent : DoBasicEvents
    {
        protected class eventStruct
        {
            ...
            public eventStruct(TreeNodeInterface toDo)
            {
                ...
                helper = toDo.getData<Transform>(structProperty.helper);
        ...
        override public void doSth(TreeNodeInterface toDo)
        {
            ...
            switch (e.type)
            {
                ...
                case eventType.choicePanel: pushNow(true); break;
                ...
                case eventType.enableAttack: pushNow(true); a.enableAttack(); break;
                case eventType.disableAttack: pushNow(true); a.disableAttack(); break;
                ...
                case eventType.showSavePanel:                   
                    a.showSavePanel(delegate { pushNow(true); });
                    break; 
                ...
                case eventType.debugLog:
                    Debug.Log(a.findText(e.str1,language.chinese));
                    pushNow(true); break;
                ...
                case eventType.playSE:
                    a.playSE(e.str1, a.findPitch(e.str1));
                    pushNow(true); break;
                case eventType.showPic:
                    if(e.helper==null)
                        a.showPicPanel(e.str1, e.vec1, a.findImg(e.str2), e.float1 / 255.0f);
                    else
                        a.showPicPanel(e.str1, e.helper.position, a.findImg(e.str2), e.float1 / 255.0f);
                    pushNow(true); break;
                case eventType.showButton:
                    if (e.helper == null)
                        a.showButton(e.str1, e.vec1, e.str2, delegate { a.ToBeNow = toDo; });
                    else
                        a.showButton(e.str1, e.helper.position, e.str2, delegate { a.ToBeNow = toDo; });
                    pushNow(true);
                    break;
                case eventType.movePic:
                    if (e.helper == null)
                        ...
                        a.movePicPanel(e.str1, e.helper.position, e.float1 / 255.0f, e.float2);
                    if (e.bool1)
                ...
                case eventType.hideButton:
                    pushNow(true);
                    a.hideButton(e.str1); break;
                case eventType.textSpecial:
                    ...
                    if(e.helper==null)
                        ...
                        showSpecialTextPanel(a, text, e.helper.position, a.findAudio(e.str2), a.findPitch(e.str2));
                    break;
                case eventType.changeScene:
                    pushNow(true);
                    changeSceneDo csd;
                    if (e.helper == null)
                        csd = new changeSceneDo(a.getTeam()[0], e.vec1,e.vec2, e.turn);
                    else
                        csd = new changeSceneDo(a.getTeam()[0], e.helper.position, e.helper.rotation.eulerAngles, e.turn);
                    a.loadLevel(e.str1, csd);
                    break;
                case eventType.playAnima:
                    GameObject prefab;
                    if (e.helper == null)
                        prefab = spawnPrefab(e.str1, e.vec1);
                    else
                        prefab = spawnPrefab(e.str1, e.helper.position);
                    ...
                    {
                        if (clips[i].name == e.str2)
                        {
                            find = true;
                            var speed = animator.GetCurrentAnimatorStateInfo(0).speed;
                            var time = clips[i].length / (speed * speed);
                            animator.Play(e.str2, 0);
                            if (e.bool1)
                                waitForTime(time);
                            else
                                pushNow(true);
                    ...
                    if (!find)
                    {
                        Debug.LogError("未找到动画");
                        pushNow(true);
                    }
                    break;
                case eventType.actorPlayAnima:
                    GameObject g = null; // 注意find函数无法查找隐藏的物体，需要给物体添加父物体以设置其可见性，且父子物体不可重名
                ...
                case eventType.changeThingPos:
                    ...
                    else
                    {
                        if (e.helper == null)
                            ...
                            changeThingPos(g, e.helper.position);
                    }
                    break;
                case eventType.moveCamera:
                    if (e.helper == null)
                        ...
                        CameraMoveToSw(e.helper.position, e.float1);
                    break;
                case eventType.sbMoveToSw:
                    ...
                    if (e.findType == findType.tag)
                        ...
                        g = GameObject.Find(e.str1);
                    #if use2D
                    if (g.GetComponent<ActorInterface2D>() == null)
                    ...
                    else
                    {
                        if (e.helper == null)
                            ...
                            SbMoveToSw(g, e.helper.position, e.turn);
                    }
                    break;
                case eventType.changeTurn2D:
                    g = null;
                    if (e.findType == findType.tag)
                        g = GameObject.FindGameObjectWithTag(e.str1);
                    else if (e.findType == findType.name)
                    ...
                    else
                    {
                        ...
                        if (g.GetComponent<ActorInterface2D>() == null)
                            g = g.transform.GetChild(0).gameObject;
                        setActorTurn2D(g, e.turn);
#endif
                    }
                    break;
                case eventType.changeTurn3D:
                    g = null;
                    if (e.findType == findType.tag)
                        g = GameObject.FindGameObjectWithTag(e.str1);
                    else if (e.findType == findType.name)
                    ...
                    else
                    {
                        if (g.GetComponent<ActorInterface>() == null)
                            g = g.transform.GetChild(0).gameObject;
                        setActorTurn3D(g, e.vec1.x,e.vec1.z);
                    }
                    break;
