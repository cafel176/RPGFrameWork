...
namespace UI
{
    ...
    public abstract class keyBoardMenu : basePanel, keyboardMenuInterface
    {
        ...
        public listAxis scaleAxis = listAxis.Vertical;
        public event Func afterDisEvent;
        ...
        public override void doEveryFrame()
        {
            ...
            if (changeScale)
            {
                ...
                if (ableTimer >= ableTime)
                {
                    ...
                    bool check;
                    if (scaleAxis == listAxis.Horizontal)
                        check = (Mathf.Abs(targetScale.x) <= 0.001f);
                    else
                        check = (Mathf.Abs(targetScale.y) <= 0.001f);

                    if (check)
                    {
                        if(afterDisEvent!=null)
                            afterDisEvent();
