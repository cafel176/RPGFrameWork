...
namespace BaseData
{
    ...
    public class actorComponentData : myListStructNode
    {
        ...
        public static string dataTypeToName(actorComponentType _type)
        {
            ...
            switch (_type)
            {
                case actorComponentType.basicMove2D: t = "基础移动2D"; break;
                case actorComponentType.basicBattle2D: t = "基础战斗2D"; break;
                case actorComponentType.followBehaviour2D: t = "跟随组件2D"; break;
                case actorComponentType.basicMove3D: t = "基础移动3D"; break;
                case actorComponentType.basicBattle3D: t = "基础战斗3D"; break;
                case actorComponentType.basicMove3DFPS: t = "基础移动3DFPS"; break;
                case actorComponentType.basicBattle3DFPS: t = "基础战斗3DFPS"; break;
    ...
    public class actorDataInspector : myListInspector<actorComponentData>
    {
        ...
        protected override void drawInspector(int i, actorComponentData e)
        {
            ...
            if (e.type == actorComponentType.basicMove2D)
            {
            ...
            else if (e.type == actorComponentType.basicBattle2D)
            {

            }
            else if (e.type == actorComponentType.followBehaviour2D)
            {
                e.bool1 = EditorGUILayout.Toggle("可以奔跑跟随", e.bool1);
            }
            else if (e.type == actorComponentType.basicMove3D)
            {
                e.float1 = EditorGUILayout.FloatField("行走速度", e.float1);
                e.str1 = EditorGUILayout.TextField("行走音效", e.str1);
                e.bool1 = EditorGUILayout.Toggle("可以奔跑", e.bool1);
                ...
                e.bool2 = EditorGUILayout.Toggle("可以跳跃", e.bool2);
                if (e.bool2)
                {
                    e.float3 = EditorGUILayout.FloatField("跳跃力度", e.float3);
                    e.str3 = EditorGUILayout.TextField("跳跃音效", e.str3);
                }
            }
            else if (e.type == actorComponentType.basicBattle3D)
            {

            }
            else if (e.type == actorComponentType.basicMove3DFPS)
            {
                e.float1 = EditorGUILayout.FloatField("行走速度", e.float1);
                e.str1 = EditorGUILayout.TextField("行走音效", e.str1);
                e.bool1 = EditorGUILayout.Toggle("可以奔跑", e.bool1);
            ...
            else if (e.type == actorComponentType.basicBattle3DFPS)
            {
                e.str1 = EditorGUILayout.TextField("子弹prefab", e.str1);
                e.float1 = EditorGUILayout.FloatField("攻击间隔", e.float1);
                e.float2 = EditorGUILayout.FloatField("攻击力度", e.float2);
            }
        }
    }
#endif
